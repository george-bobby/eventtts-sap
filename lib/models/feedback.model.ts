...
export interface IFeedbackTemplate extends Document {
  ...
  feedbackHours: number; // Hours after event ends to send feedback
  isActive: boolean;
  createdAt: Date;
  updatedAt: Date;
}
...
const feedbackTemplateSchema = new Schema<IFeedbackTemplate>(
  {
    ...
    feedbackHours: { type: Number, required: true, default: 2 },
...
// Models
const FeedbackTemplate = models.FeedbackTemplate || model('FeedbackTemplate', feedbackTemplateSchema);
const FeedbackResponse = models.FeedbackResponse || model('FeedbackResponse', feedbackResponseSchema);
const EmailSchedule = models.EmailSchedule || model('EmailSchedule', emailScheduleSchema);
