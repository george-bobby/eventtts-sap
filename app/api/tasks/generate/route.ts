...
export async function POST(request: NextRequest) {
  try {
    ...
    const eventContext = eventDetails ? `
      Event Details:
      - Location: ${eventDetails.isOnline ? 'Online Event' : eventDetails.location || 'TBD'}
      - Capacity: ${eventDetails.capacity || 'Unlimited'}
      - Duration: ${eventDetails.startDate && eventDetails.endDate 
        ? `${new Date(eventDetails.startDate).toLocaleDateString()} to ${new Date(eventDetails.endDate).toLocaleDateString()}`
        : 'TBD'}
      - Pricing: ${eventDetails.isFree ? 'Free Event' : `₹${eventDetails.price || 0} per ticket`}
    ...
    // Generate tasks using AI SDK
    const { object: tasksData } = await generateObject({
    ...
    const tasksWithIds = tasksData.tasks.map((task, index) => ({
    ...
    const saveResult = await saveEventTasks(eventId, userId, tasksWithIds);
    ...
    const fallbackTasks = getFallbackTasks(isSubEvent);
    
    try {
      await saveEventTasks(eventId, userId, fallbackTasks);
      return NextResponse.json({
        success: true,
        tasks: fallbackTasks,
...
function getFallbackTasks(isSubEvent?: boolean) {
  ...
  if (isSubEvent) {
    return [
      ...
      },
      {
        ...
        subtasks: [
          { id: `fallback_sub_${baseId}_2_1`, content: 'Define space requirements' },
      ...
      },
      {
        ...
        subtasks: [
          { id: `fallback_sub_${baseId}_3_1`, content: 'Create presentation materials' },
  ...
  return [
    ...
    },
    {
      ...
      subtasks: [
        { id: `fallback_sub_${baseId}_2_1`, content: 'Research and book venue' },
    ...
    },
    {
      ...
      subtasks: [
        { id: `fallback_sub_${baseId}_3_1`, content: 'Create promotional materials' },
        { id: `fallback_sub_${baseId}_3_2`, content: 'Set up social media campaigns' },
        { id: `fallback_sub_${baseId}_3_3`, content: 'Reach out to media contacts' },
      ],
    },
    {
      id: `fallback_${baseId}_4`,
      ...
      subtasks: [
        ...
        { id: `fallback_sub_${baseId}_4_2`, content: 'Prepare presentation materials' },
    ...
    },
    {
      ...
      subtasks: [
        { id: `fallback_sub_${baseId}_5_1`, content: 'Conduct final venue walkthrough' },
