...
export async function GET(
  ...
  try {
    ...
    if (photosOnly) {
      ...
      // Get gallery info
      const gallery = await getPhotoGallery(params.galleryId, userId);
...
export async function POST(
  ...
  try {
    ...
    for (const photo of photos) {
      if (!photo.fileName || !photo.originalName || !photo.fileUrl || !photo.fileSize || !photo.mimeType || !photo.dimensions) {
        return NextResponse.json(
