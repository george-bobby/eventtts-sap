...
export async function POST(request: NextRequest) {
  try {
    ...
    const { type, templateId, stakeholderId, stakeholderIds, fieldValues, defaultFieldValues } = body;
    ...
    if (type === 'single') {
      ...
      if (!stakeholderId || !fieldValues) {
        return NextResponse.json(
          { error: 'Stakeholder ID and field values are required for single generation' },
          { status: 400 }
        );
      }
