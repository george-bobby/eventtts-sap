  ...
  try {
    ...
    if (includeGalleryLink) {
      ...
      if (galleries.length > 0) {
        ...
        const publicGallery = galleries.find(g => g.visibility === 'public') || galleries[0];
    ...
    if (type === 'certificate') {
      ...
      for (const result of results) {
        if (result.success) {
          const stakeholder = stakeholders.find(s => s.email === result.email);
          if (stakeholder) {
            await connectToDatabase();
      ...
      const thankYouEmails: ThankYouEmailData[] = stakeholders.map(stakeholder => ({
        eventTitle: event.title,
        eventId: event._id,
        participantName: stakeholder.name,
        participantEmail: stakeholder.email,
        ...
        eventDate: event.startDate.toDateString(),
        galleryUrl: galleryUrl || undefined,
        certificateUrl: (includeCertificate && stakeholder.certificateId?.certificateUrl) 
